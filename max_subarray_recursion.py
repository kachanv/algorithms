...
def max_subarray_recursion(arr: list, low: int, high: int) -> list:
    if high == low:  # only 1 element in arr
        ...
        left_low, left_high, left_sum = max_subarray_recursion(arr, low, mid)  # calling recursion on the left half of the array
        right_low, right_high, right_sum = max_subarray_recursion(arr, mid + 1, high)  # calling recursion on the right half of the array
        cross_low, cross_high, cross_sum = max_cross_subarray(arr, low, mid, high)  # finding the maximum subarray that intersects the midpoint
